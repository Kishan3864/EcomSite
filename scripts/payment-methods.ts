 ...
 * able to take money — UPI needs UPI_VPA in .env, the gateway needs the
 * Razorpay keys — and the checkout hides any method that cannot (see
...
async function main() {
  ...
  const gatewayReady = !!process.env.RAZORPAY_KEY_ID?.trim() && !!process.env.RAZORPAY_KEY_SECRET?.trim();
  ...
  console.log(gatewayOn && gatewayReady ? ok("Pay online — Razorpay") : off("Pay online — Razorpay"));
  ...
  if (gatewayOn && !gatewayReady) console.log(warn("The gateway is switched on but the Razorpay keys are missing, so it stays hidden."));
