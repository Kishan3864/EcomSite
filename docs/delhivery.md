...
Delhivery runs two identical systems:

| | Host | Token | Effect |
|---|---|---|---|
| **staging** | `staging-express.delhivery.com` | the *test* token | Bookings get a real-looking waybill and tracking; **no courier is dispatched, nothing is billed** |
| **production** | `track.delhivery.com` | the *live* token | Real shipments, real pickups, real bills |

The shop picks the system from one variable. What you prove on staging is
exactly what runs live — the code path is the same, only the host and token
differ.

### 1. Get the two things from Delhivery One

1. **API token** — Delhivery One → *Settings* → *API* (their menu may call it
   *Integrations* or *Developer*). There is a token for the test system and a
   separate one for live. Start with the test token.
2. **Pickup location name** — Delhivery One → *Settings* → *Pickup locations*
   (or *Warehouses*). Copy the **name** of your registered address exactly,
   including capitals and spaces. Delhivery matches it letter for letter; a
   booking with a name it does not know is refused.

If the test system has no pickup location yet, register one there too — the
staging panel is separate from the live one.
...
DELHIVERY_API_TOKEN="<the test token>"
...
The check script prints which environment it is on, proves the token by asking
Delhivery for one waybill, and looks up a pincode (pass another as the first
argument). On staging the waybill costs nothing. If it says the token was
rejected, the token is wrong or it belongs to the other environment. If it
cannot connect, the server could not reach Delhivery — the same network the
Google sign-in notes describe.

### 3. Book a test parcel

1. Place an order on the site (cash on delivery is simplest — an online order
   must be paid before it can be booked).
2. Open it in `/admin/orders`. The *Shipment* card shows **Delhivery — Test mode**
   and a **Book with Delhivery** button. Press it.
3. The order now has a waybill, its status is *packed*, and the customer's
   order page shows the booking on the timeline.
4. **Request pickup** for tomorrow; **Label** opens the PDF; **Refresh tracking**
   pulls whatever the test system has (on staging that is usually just
   "Manifested" — the test system does not simulate a journey).

If a booking is refused, the message on the card is Delhivery's own reason —
most often an unknown pickup location name, a pincode they do not serve, or a
phone number that is not ten digits.

### 4. Switch it live

Change the two lines in `.env` and reload — nothing else:

```
DELHIVERY_ENV="production"
DELHIVERY_API_TOKEN="<the live token>"
```
...
Note that on production the check script's waybill fetch uses one number from
your allocation; Delhivery allots them in the thousands, so it does not matter,
but do not run it in a loop.

From then on the card says **Delhivery — Live**, and every *Book with Delhivery*
is a real shipment on your account.
