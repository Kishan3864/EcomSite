...
function Group({
  ...
  return (
    <section className="border-b border-hairline py-4 last:border-b-0">
      ...
      >
        <span className="text-[12px] font-semibold uppercase tracking-[0.1em] text-ink-900">
          {title}
...
function CheckRow({
  ...
  return (
    <label className="group flex cursor-pointer items-center gap-2.5 py-1.5">
      ...
      <span className="flex-1 truncate text-[13px] text-ink-700 group-hover:text-ink-950">
        {label}
      </span>
      {count != null && (
...
export function FilterPanel({
  ...
  return (
    <div className="text-ink-900">
      <div className="flex items-center justify-between gap-3 border-b border-hairline pb-3">
        ...
        {active.length > 0 && (
          <button
            ...
            className="inline-flex items-center gap-1 text-[12px] font-medium text-sale-600 hover:underline"
          >
        ...
        <Group title="Brand" count={query.brands?.length}>
          ...
          {facets.brands.length > 8 && (
            <button
              ...
              className="mt-2 text-[12px] font-medium text-brand-700 hover:underline"
            >
      ...
      {onDone && (
        <div className="sticky bottom-0 -mx-5 mt-2 border-t border-hairline bg-surface px-5 py-3">
      ...
      <div className="flex items-center gap-2">
        <label className="flex-1">
          ...
          <input
            ...
            className="h-9 w-full rounded-lg border border-ink-200 bg-surface px-2.5 text-[13px] tabular-nums outline-none focus:border-brand-500"
          />
        </label>
    ...
    <div className="flex flex-wrap items-center gap-1.5">
      {chips.map((chip) => (
        <button
          ...
          className="inline-flex items-center gap-1.5 rounded-full border border-ink-200 bg-surface py-1.5 pl-3 pr-2 text-[12px] font-medium text-ink-700 transition-colors hover:border-sale-500 hover:text-sale-600"
        >
          {chip.label}
          <X size={12} />
        </button>
      ...
      <button
        ...
        className="ml-1 text-[12px] font-medium text-ink-500 underline-offset-2 hover:text-sale-600 hover:underline"
      >
        Clear all
      </button>
    </div>
