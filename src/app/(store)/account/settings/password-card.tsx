...
function SendButton() {
  ...
  return (
    <Button type="submit" size="md" loading={pending}>
      <KeyRound size={16} /> {pending ? "Sending…" : "Email me a link"}
    </Button>
  );
}
...
export function PasswordCard({ email }: { email: string }) {
  ...
  if (state.ok) {
    return (
      <div role="status" className="card-muted flex items-start gap-3 p-4">
  ...
  return (
    <Form action={action}>
      ...
      {state.error ? (
        <p role="alert" className="mt-3 rounded-md bg-gold-50 px-3.5 py-2.5 text-[13px] leading-[1.5] text-ink-800 ring-1 ring-inset ring-gold-200">
          {state.error}
        </p>
      ) : null}
      <div className="mt-4">
